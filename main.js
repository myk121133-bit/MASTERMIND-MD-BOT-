...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [MASTERMIND-MD] ${new Date().toISOString()}: ${message}`);
}
...
async function arslanPair(number, res = null) {
    ...
    try {
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        ...
                        caption: `\n╭────────────────────◇\n│✦ *MASTERMIND-MD — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*© Powered by MASTERMIND-MYK*`
                    });
                }
            }
            if (connection === 'close') {
            ...
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© MASTERMIND-MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN: MASTERMIND-MD BOY\nORG: MASTERMIND-MD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© MASTERMIND-MD' } }
                    }},
        ...
        arslanLog(`MASTERMIND-MD Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 MASTERMIND-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
